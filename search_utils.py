...
_SYNONYM_MAP = {
    ...
    "打针": "注射", "扎针": "注射", "挨针": "注射", "打了一针": "注射",
    # === 产品动作 ===
    "做": "操作", "做了": "操作", "弄": "操作", "搞": "操作",
    # === 疼痛 ===
    "痛": "疼痛", "疼": "疼痛", "刺痛": "疼痛", "酸痛": "疼痛", "胀痛": "疼痛",
    "好痛": "疼痛", "好疼": "疼痛", "很痛": "疼痛", "很疼": "疼痛",
    "有点疼": "疼痛", "有点痛": "疼痛",
    # === 肿胀 ===
    ...
    "不可以做": "禁忌", "不建议做": "禁忌", "做不了": "禁忌",
    "不让做": "禁忌", "不行": "禁忌",
    # === 效果 ===
    "见效": "效果", "有用": "效果", "管用": "效果", "有效": "效果",
    "有用吗": "效果", "管不管用": "效果", "好使": "效果", "靠谱": "效果",
    "有没有效": "效果", "看得出": "效果", "明显": "效果",
    # === 反应 ===
    "术后反应": "不良反应", "副作用": "不良反应",
    "有反应": "不良反应", "出问题": "不良反应", "出事": "不良反应",
    # === 麻醉 ===
    "麻药": "麻醉", "敷麻": "麻醉", "敷麻药": "麻醉", "表麻": "表面麻醉",
    "涂麻药": "麻醉", "上麻药": "麻醉",
    # === 洗浴 ===
    "洗澡": "洗浴", "泡澡": "洗浴", "淋浴": "洗浴",
    "能洗澡吗": "洗浴", "可以洗澡": "洗浴",
    # === 护理 ===
    "保养": "护理", "养护": "护理",
    "注意啥": "注意事项", "注意什么": "注意事项", "有啥讲究": "注意事项",
    # === 脸部 ===
    "脸": "面部", "面": "面部", "脸上": "面部", "面上": "面部",
    # === 皱纹 ===
    "纹": "皱纹", "细纹": "皱纹", "法令纹": "皱纹",
    "有纹了": "皱纹", "长纹": "皱纹",
    # === 紧致 ===
    ...
    "肉毒": "肉毒素注射", "肉毒素": "肉毒素注射", "肉毒杆菌": "肉毒素注射",
    "保妥适": "肉毒素注射", "衡力": "肉毒素注射", "乐提葆": "肉毒素注射",
    ...
    "脸垮": "皮肤松弛", "垮了": "皮肤松弛", "松松垮垮": "皮肤松弛",
    "暗沉": "肤色不均", "暗黄": "肤色不均", "发黄": "肤色不均",
    "气色差": "肤色不均", "没光泽": "肤色不均", "肤色暗": "肤色不均",
    ...
    "能喝咖啡": "咖啡", "能抽烟": "禁烟",
    "晒太阳": "防晒", "日晒": "防晒",
    "冰敷": "冷敷", "热敷": "热敷护理", "敷面膜": "面膜",
    "能敷面膜吗": "面膜", "能贴面膜吗": "面膜",
    "几天能上班": "恢复期", "几天能出门": "恢复期",
    "能碰吗": "触碰禁忌", "能摸吗": "触碰禁忌", "能揉吗": "按摩禁忌",
    "能按摩吗": "按摩禁忌", "能做面部按摩吗": "按摩禁忌",
    "能泡温泉吗": "高温禁忌", "能蒸桑拿吗": "高温禁忌",

    # === 价格/费用口语（辅助路由检测，不影响检索内容） ===
    "贵不贵": "价格", "收费": "价格", "多少钱": "价格", "划算": "价格",
    "费用": "价格", "花多少": "价格", "什么价位": "价格",

    # === 口语化表达 → 规范术语（补充） ===
    "脸僵了": "表情不自然", "脸僵": "表情不自然", "笑不动": "表情不自然",
    "表情僵硬": "表情不自然", "不自然": "表情不自然",
    ...
    "有硬块": "硬结", "硬疙瘩": "硬结",
    "鼓包": "凸起", "鼓起来": "凸起", "凸出来": "凸起",
    "吸收了": "代谢吸收", "被吸收": "代谢吸收", "没效果了": "代谢吸收",

    # === 时间口语 → 规范表达 ===
    "刚打完": "术后即刻", "刚做完": "术后即刻", "刚做好": "术后即刻",
    "过了一周": "术后7天", "一周了": "术后7天", "7天了": "术后7天",
    "过了一个月": "术后1个月", "一个月了": "术后1个月",
    "半年了": "术后6个月", "一年了": "术后12个月",
    "第二天": "术后1天", "隔天": "术后1天",
    "三天了": "术后3天", "一周后": "术后7天",

    # === 复合症状口语 ===
    "又红又肿": "红肿", "红肿热痛": "红肿 疼痛", "又红又硬": "红肿 硬结",
    "又疼又肿": "疼痛 肿胀", "又青又肿": "淤青 肿胀",
    "红肿疼": "红肿 疼痛", "肿了疼": "肿胀 疼痛",

    # === 并发症口语 ===
    "栓塞": "血管栓塞", "打到血管": "血管栓塞", "堵了": "血管栓塞",
    "皮肤发白": "血管栓塞", "视力模糊": "血管栓塞",
    "长肉芽": "肉芽肿", "起疙瘩": "肉芽肿",
    "过敏了": "过敏反应", "起红疹": "过敏反应",
    "发烧了": "感染", "流脓": "感染", "化脓了": "感染",
    ...
    # === 注射部位口语（补充） ===
    "泪沟": "泪沟", "卧蚕": "下睑", "木偶纹": "口角纹",
    "嘴唇打": "唇部填充", "丰下巴": "下颏填充", "丰额头": "额部填充",
    "隆鼻": "鼻部填充", "垫下巴": "下颏填充",

    # === 疗程相关口语 ===
    "要做几次": "疗程次数", "做几次": "疗程次数", "需要几针": "疗程次数",
    "隔多久做": "疗程间隔", "多久做一次": "疗程间隔",
    "做完多久见效": "起效时间", "什么时候见效": "起效时间",
    "效果能维持多久": "维持时间", "能管多久": "维持时间",

    # === 产品错别字/变体 ===
    "非罗奥": "菲罗奥", "菲洛奥": "菲罗奥", "非洛奥": "菲罗奥",
    "飞罗奥": "菲罗奥", "妃罗奥": "菲罗奥",
    "赛罗菲": "赛洛菲", "塞罗菲": "赛洛菲",
    "赛诺菲": "赛洛菲", "塞诺菲": "赛洛菲",
}
        ...
        if matched:
            for syn in synonyms:
                if syn not in q_lower:
                    extra.add(syn)
        # 反向部分匹配：查询中的中文子串是同义词 key 的一部分
        # 例如 query="蛋白有什么用" → term="胶原蛋白" 不在 query 中，
        # 但 "蛋白" 在 "胶原蛋白" 中 → 扩展 "胶原蛋白" 及其同义词
        elif len(term) >= 3 and _RE_CJK_WORD.match(term):
            # 从 query 中提取连续中文片段，看是否是 term 的子串
            for seg in _extract_cjk_segments(q_lower):
                if len(seg) >= 2 and seg in term and seg != term:
                    extra.add(term)
                    for syn in synonyms:
                        if syn not in q_lower:
                            extra.add(syn)
...
# 预编译：提取连续中文字符片段
_RE_CJK_SEGMENT = re.compile(r"[\u4e00-\u9fff]+")


def _extract_cjk_segments(text: str) -> List[str]:
    """从文本中提取所有连续中文片段。"""
    return _RE_CJK_SEGMENT.findall(text)
